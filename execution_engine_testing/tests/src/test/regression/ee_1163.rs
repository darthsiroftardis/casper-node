...
use casper_types::{account::AccountHash, runtime_args, system::{handle_payment, mint}, Gas, Motes, RuntimeArgs, U512, SystemConfig, MintCosts};
...
fn shouldnt_consider_gas_price_when_calculating_minimum_balance() {
    ...
    let create_account_request = {
        let transfer_amount = Motes::new(U512::from(MintCosts::default().transfer) + U512::one());
